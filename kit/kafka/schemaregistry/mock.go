...
func (c *mockclient) Register(subject string, schema SchemaInfo, normalize bool) (int, error) {
...
func (c *mockclient) GetBySubjectAndID(subject string, id int) (SchemaInfo, error) {
	cacheKey := subjectID{
		subject: subject,
		id:      id,
	}
...
// Returns SchemaMetadata object.
func (c *mockclient) GetSchemaMetadata(subject string, version int) (SchemaMetadata, error) {
	var json string
	...
	var info SchemaInfo
	if err := info.UnmarshalJSON([]byte(json)); err != nil {
...
func (c *mockclient) GetAllVersions(subject string) ([]int, error) {
...
func (c *mockclient) allVersions(subject string) []int {
	versions := make([]int, 0)
	c.versionCacheLock.RLock()
	for key, value := range c.versionCache {
...
func (c *mockclient) latestVersion(subj string) int {
	versions := c.allVersions(subj)
	if len(versions) == 0 {
		return -1
	}
...
func (c *mockclient) DeleteSubjectVersion(subject string, version int, permanent bool) (int, error) {
	c.versionCacheLock.Lock()
...
func (c *mockclient) GetCompatibility(subject string) (Compatibility, error) {
...
func (c *mockclient) UpdateCompatibility(subj string, upd Compatibility) (Compatibility, error) {
	c.compatibilityCacheLock.Lock()
	c.compatibilityCache[subj] = upd
	c.compatibilityCacheLock.Unlock()
	return upd, nil
}
...
func (c *mockclient) TestCompatibility(subj string, version int, schema SchemaInfo) (bool, error) {
...
func (c *mockclient) GetDefaultCompatibility() (Compatibility, error) {
	c.compatibilityCacheLock.RLock()
...
func (c *mockclient) UpdateDefaultCompatibility(update Compatibility) (Compatibility, error) {
	c.compatibilityCacheLock.Lock()
